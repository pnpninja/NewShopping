...
def interleave(similarity_recommended_items, popularity_recommended_items):
    ...
    while similarity_index < len(
        similarity_recommended_items
    ) and popularity_index < len(popularity_recommended_items):
        if current % ratio == 0:
...
def get_coldstart_recommendation(
    data, user_column, product_column, freq_column, k, user_id
):
...
def get_best_k_items(data, user_column, item_column, freq_column, k, user_id):
    ...
    if check_coldstart(data, user_column, item_column, freq_column, k, user_id):
        return get_coldstart_recommendation(
            data, user_column, item_column, freq_column, k, user_id
        )
    # user-item interactions table
    interactions = pd.pivot_table(
        data, values=freq_column, index=user_column, columns=item_column
    )
    ...
    if np.min(np.min(interactions)) == np.max(np.max(interactions)):
        ...
        interactions = (interactions - np.min(np.min(interactions))) / (
            np.max(np.max(interactions)) - np.min(np.min(interactions))
        )
    # create dummy id, in order to columnize userId
    training_data = interactions.reset_index()
    training_data.index.names = [freq_column]
    training_data = pd.melt(
        training_data, id_vars=[user_column], value_name=freq_column
    ).dropna()
    user_id = [user_id]
    recommender = tc.recommender.item_similarity_recommender.create(
        tc.SFrame(training_data),
        user_id=user_column,
        item_id=item_column,
        target=freq_column,
        verbose=False,
...
def get_best_k_merchants(data, user_column, merchant_column, freq_column, k, user_id):
    ...
    if check_coldstart(data, user_column, merchant_column, freq_column, k, user_id):
        return get_coldstart_recommendation(
            data, user_column, merchant_column, freq_column, k, user_id
        )

    # user-merchant interactions table
    interactions = pd.pivot_table(
        data, values=freq_column, index=user_column, columns=merchant_column
    )
    ...
    similarity_recommender = tc.recommender.item_similarity_recommender.create(
        tc.SFrame(training_data),
        user_id=user_column,
        item_id=merchant_column,
        target=freq_column,
        verbose=False,
    )

    similarity_recommendation = similarity_recommender.recommend(
        users=user_id, k=similarity_k, verbose=False
    )

    popularity_recommender = tc.recommender.popularity_recommender.create(
        tc.SFrame(training_data),
        user_id=user_column,
        item_id=merchant_column,
        target=freq_column,
    ...
    popularity_recommendation = popularity_recommender.recommend(
        users=user_id, k=popularity_k, verbose=False
    )

    similarity_recommended_items = [
        a for a in similarity_recommendation[merchant_column]
    ]
    popularity_recommended_items = [
        a for a in popularity_recommendation[merchant_column]
    ]

    recommended_items = interleave(
        similarity_recommended_items, popularity_recommended_items
    )
...
def main():
    ...
    data = pd.read_csv(
        "music_data.csv",
        usecols=["userId", "artistId", "plays"],
        encoding="unicode_escape",
    )
    artist_data = pd.read_csv(
        "music_data.csv", usecols=["artistId", "artist"], encoding="unicode_escape"
    )
    ...
    recommendations = get_best_k_items(
        ...
        item_column=artist_column,
        freq_column=plays_column,
        k=10,
        user_id=user_id,
    )
    ...
    for a in artists:
        artistId_to_artistName[a] = artist_data.loc[
            artist_data["artistId"] == a, "artist"
        ].unique()[0]
